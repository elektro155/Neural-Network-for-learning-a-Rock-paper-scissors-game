...
public:
	...
	int simplyThink(int mine, int opp){ //the data just go through the neural network with fixed values for each neuron
		int result;
	...
	void learn(){
		int mine=1; //val 1-3
		int opp =1;  //val 1-3
		int max = 4;//for max value of being checked coefficients 
			...
			for (int j = 1; j < 4; j++){//all possibilities for opponent
				for (int a = 0; a < max; a++){			//1
					for (int b = 0; b < max; b++){			//2
						for (int c = 0; c < max; c++){			//3
							for (int d = 0; d < max; d++){			//4
								for (int e = 0; e < max; e++){			//5
									for (int f = 0; f < max; f++){			//6
										for (int g = 0; g < max; g++){			//7
											for (int h = 0; h < max; h++){			//8
												for (int k = 0; k < max; k++){			//9
													for (int l = 0; l < max; l++){			//10
														for (int m = 0; m < max; m++){			//11
															for (int n = 0; n < max; n++){			//12
																...
																int difference;
																difference = abs(simplyThink(i, j) - (int)result(i, j)); //difference between calculated result and expected one
																if (difference == 0){
	...
	void play(){
		...
		N4.val[3] = 3;
		int a, b,result;
		cout << "1 dla kamienia 2 dla no¿yczek 3 dla papieru - ty" << endl;
		cin >> a;
		cout << "1 dla kamienia 2 dla no¿yczek 3 dla papieru - przeciwnik" << endl;
		cin >> b;
